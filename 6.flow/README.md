...
* 初始化参数: 从配置文件和shell语句中读取参数合并参数, 得出最终的参数
* 开始编译: 用上一步的参数初始化Comolier对象, 加载所有的配置插件,执行对象的run方法,执行配置文件根据entry找出所有入口文件
* 编译模块: 从入口文件触发,调用所有配置Loader对模块编译,再找出该模块依赖的模块, 在递归本步奏直到所有入口依赖的文件都经过本步奏处理
* 完成模块的编译: 进过Loder编辑后,得到所有的模块编译后的内容和他们之间的所有依赖关系
* 输出资源: 根据入口和模块之间的依赖关系, 组成一个个包含多个模块的chunk, 再把每个chunk转换成一个单独的文件加入输出列表,这是可以修改文件内容的最后机会
* 输出完成: 确认好输出内容后,根据配置确认输出的路径和文件名,把文件内容写入文件系统
